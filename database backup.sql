...
/*!50001 CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `parentviews` AS select `p`.`id` AS `id`,concat(`u`.`fname`,' ',`u`.`lname`) AS `name`,`u`.`mobile` AS `mobile`,`u`.`status` AS `status`,`u`.`email` AS `email`,`p`.`id_verified_flag` AS `id_verified_flag`,count(`s`.`primary_parent_id`) AS `children_count` from ((`parents` `p` join `users` `u` on(`p`.`user_id` = `u`.`id`)) join `students` `s` on(`s`.`primary_parent_id` = `p`.`user_id`)) group by `p`.`id` */;
