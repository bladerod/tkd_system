                ...
                <div class="card z-index-2 bg-white rounded-xl shadow-sm">
                    ...
                    <div class="" style="border-top: 1px solid rgba(0, 0, 0, 0.1);">
                        ...
                        <div class="m-3 flex justify-end ">
                            <button @if ($canCreateInstructor) @click="openAdd()" @else hidden @endif
                                class="bg-green-700 text-white px-4 py-3 rounded-xl hover:bg-green-600">
                                <i class="fa fa-plus mr-2"></i>Add Instructor
                            </button>
                        </div>

                        <form method="GET" action="{{ route('instructor.index') }}" class="p-4 mx-4 ">
                            ...
                            <div class="grid grid-cols-12 gap-4 items-end">
                                <div class="col-span-3">
                                    <label
                                        class="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Account
                                        Status</label>
                                    <select name="status"
                                ...
                                <div class="col-span-3">
                                    <label
                                        class="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Role</label>
                                ...
                                <div class="col-span-4">
                                    <label
                                        class="block text-xs font-medium text-gray-500 mb-1 uppercase tracking-wider">Quick
                                        Search</label>
                                    <div class="flex gap-2">
                                        <input type="text" id="jqSearchInput" placeholder="Search name, contact..."
                                            class="w-full px-4 py-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#1C1C1D] focus:border-transparent transition-all duration-200 bg-white">
                                        <button type="button" id="jqSearchBtn"
                                            class="bg-gray-800 text-white px-4 py-2.5 rounded-lg hover:bg-gray-900 transition-all duration-200 font-medium text-sm">
                                ...
                                <div class="col-span-2">
                                    <div class="flex gap-2">
                                        <button type="submit"
                                            class="flex-1 bg-[#1C1C1D] text-white px-3 py-2.5 rounded-lg hover:bg-[#2C2C2D] transition-all duration-200 font-medium text-sm shadow-sm flex items-center justify-center gap-2">
                                            <i class="fas fa-filter"></i> Filter
                                        </button>
                                        <a href="{{ route('instructor.index') }}"
                                            class="px-3 py-2.5 bg-gray-200 text-gray-700 rounded-lg hover:bg-gray-300 transition-colors text-sm font-medium flex items-center justify-center">
                                            Clear
                                        </a>
                                    </div>
                                </div>
                                        ...
                                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="flex items-center gap-3">
                                                    <!-- EDIT -->
                                                    <button @if ($canEditInstructor) @click.prevent="openEdit({
                                                        id: '{{ $inst->id }}',
                                                        fname: '{{ $inst->fname }}',
                                                        lname: '{{ $inst->lname }}',
                                                        email: '{{ $inst->email }}',
                                                    ...
                                                    <form action="{{ route('instructor.delete', $inst->id) }}" method="POST"
                                                        class="delete-form"
                                                        data-instructor-name="{{ $inst->fname }} {{ $inst->lname }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button @if (!$canDeleteInstructor) hidden @endif type="button"
                                                            class="bg-red-600 hover:bg-red-500 p-2.5 rounded-lg delete-instructor-btn"
                                        ...
                                        <tr>
                                            <td colspan="6" class="px-6 py-8 text-center text-gray-500">
                                                <div class="flex flex-col items-center">
                                                    ...
                                                    <p class="text-sm">Click the "Add Instructor" button to create a new
                                                        instructor.</p>
                                                </div>
                                            </td>
                                        </tr>
            ...
            <div class="modal-content relative bg-white rounded-xl shadow-lg z-10 text-base max-w-2xl w-full">
                <div class="bg-[#1C1C1D] rounded-t-xl ps-5 py-3 flex">
                    ...
                    <h2 class="text-2xl text-white font-semibold "
                        x-text="isEdit ? 'Edit Instructor' : 'Add Instructor'"></h2>
                </div>
                <div class="p-6">
                    <form :action="isEdit ? '/instructor/update/' + form.id : '/instructor/store'" method="POST"
                        enctype="multipart/form-data">
                        @csrf

                        <label for="branch">Branch</label>
                        <div>
                            <select name="branch_id" x-model="form.branch_id" class="rounded-md  py-2 mb-1"
                                    ...
                                    <template x-if="!show">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none"
                                            ...
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M13.875 18.825A10.05 10.05 0 0112 19c-5.523 0-10-4.477-10-10 0-1.191.194-2.337.55-3.422m2.816-3.164A9.97 9.97 0 0112 5c5.523 0 10 4.477 10 10 0 1.191-.194 2.337-.55 3.422m-2.816 3.164M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                        </svg>
                                    </template>
                                    <template x-if="show">
                                        <!-- Eye Open Icon -->
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-1">
                            ...
                            <div>
                                ...
                                <select name="certification_level" x-model="form.certification_level"
                                    class="w-full border rounded p-2">
                        ...
                        <div class="mb-1">
                            ...
                            <input type="text" name="specialization" x-model="form.specialization"
                                class="w-full border rounded p-2">
                        </div>

                        <div class="mb-1">
                            <label class="block text-sm">Bio</label>
                            <textarea name="bio" x-model="form.bio" class="w-full border rounded p-2 text-sm"
                                rows="3"></textarea>
                        </div>
                        ...
                        <div class="flex justify-end gap-3">
                            ...
                            <button type="submit"
                                class="px-4 py-2 bg-[#065f46] text-white rounded text-sm">Save</button>
                        </div>
